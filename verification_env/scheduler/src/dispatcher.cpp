...
std::queue<std::pair<Request *,std::chrono::time_point<std::chrono::high_resolution_clock>>> Dispatcher::sleepers;
...
void Dispatcher::sleepersHandler() {
	while (!stopSH) {
		...
		while (!sleepers.empty() && duration_cast<milliseconds>(high_resolution_clock::now() - sleepers.front().second).count()>sleepTime) {
