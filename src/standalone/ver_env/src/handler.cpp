...
int main(int argc, char **argv) {
    ...
    Server server(*n, ros::this_node::getName() + "/migrate",
    ...
    if (cooName == ros::this_node::getName()) {
        ...
        //        coordinator..." << "\n";
        sub_cs = n->subscribe("coo_stat", 100, receiveStatMSGS,
                              ros::TransportHints().tcpNoDelay());
        sub_ack = n->subscribe("coo_ack", 100, acksCB,
                               ros::TransportHints().tcpNoDelay());
        nameToPublisher["coo_stat"];
        nameToPublisher["coo_ack"];
        // independent thread containing the coordinator main loop
        ct = new boost::thread(coordinator);
    } else {
        nameToPublisher["coo_stat"] =
            n->advertise<verification_env::stat>("coo_stat", 100, 1);
        nameToPublisher["coo_ack"] =
            n->advertise<verification_env::ack>("coo_ack", 100, 1);
    }
