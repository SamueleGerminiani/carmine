...
inline void ttlInsert(const std::string &topic, const ros::Time &stamp) {
    ...
    // debug
//        topicToLatencyWindow.at(topic).push_front( (ros::Time::now() - stamp).toSec()+((double)(random()%100))/1000);
    topicToLatencyWindow.at(topic).push_front( (ros::Time::now() - stamp).toSec());
...
inline void removeCheckerCallbacks(std::unordered_set<std::string> &chsToBeRemoved) {
...
inline void start_upCB(const verification_env::su &msg) {
...
inline void acksCB(const verification_env::ack &msg) {
