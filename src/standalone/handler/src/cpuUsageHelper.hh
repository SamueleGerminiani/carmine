...
uint32_t getThiscpuFreq() {
  ...
  if (ifs.is_open()) {
    ...
    while (ifs.good()) {
      getline(ifs, line);
      int rc = pcre_exec(reCompiled, 0, line.c_str(), line.length(), 0, 0,
                         results, 10);
      if (rc < 0)
        continue;
...
inline void initCPUTot() {
  ...
  if (fscanf(file, "cpu %llu %llu %llu %llu", &lastTotalUser_TOT,
             &lastTotalUser_TOTLow, &lastTotalSys_TOT,
             &lastTotalIdle_TOT) != 4) {
    std::cout << "fscanf error exiting..."
              << "\n";
    exit(1);
  }
  fclose(file);
