  ...
  bool generateCpp(codeGenerator::SpotAutomata& fsm,std::vector<checkerVar>& varList, 
    std::pair<std::string,std::unordered_map<std::string,oden::Proposition*>>& parsedFormula, std::string& checkerName){
    ...
    if(dst.fail()){
      std::cout<<"Error: could not open output/checkers/src/"<<checkerName<<".cpp"<<std::endl;
      return false;
    }
    ...
    while (getline(src, line)){
        ...
        if(line.compare("$order_entry") == 0){
           line = "";
            for( unsigned int i = 0;i< fsm->ap().size(); i++){
                line+=codeGenerator::ident1+"if (order_entry & (1ULL << "+std::to_string(i)+")) {\n";
                line+=codeGenerator::ident2+"last_"+(fsm->ap()[i]).ap_name()+"= pbuff_entry & (1ULL << "+std::to_string(i)+");\n";
        ...
        //Code for calling eval() and resetting checker
           ...
           line = "";
           line+=codeGenerator::ident1+"if (!eval_"+checkerName+"(last_"+(fsm->ap()[0]).ap_name();
        ...
        //Set buffer received during the migration
          ...
          for(unsigned int i = 0; i<fsm->ap().size(); i++){
            line+=codeGenerator::ident3+"bool p"+std::to_string(i)+" = val & (1ULL << "+std::to_string(i)+");\n";
            placeholders+=" ,p"+std::to_string(i);
          }

          line+=codeGenerator::ident3+"assign<bool>(pbuff, index_p"+placeholders+");\n";
          line+=codeGenerator::ident3+"val = (req.buffer_o)[i];\n";

          for(unsigned int i = 0; i<fsm->ap().size(); i++){
            line+=codeGenerator::ident3+"p"+std::to_string(i)+" = val & (1ULL << "+std::to_string(i)+");\n";
          }

          line+=codeGenerator::ident3+"assign<bool>(order, index_p"+placeholders+");\n";
        ...
        //Code for generating boolean values for placeholders
            ...
            for(auto v: varList){
              ...
              line+=codeGenerator::ident5+"case "+std::to_string(count)+":{\n";
              //auto varsFromMsgType = v.second;
              //Assign the message's value to the related static variables 
              /*for(auto var : varsFromMsgType){
                line+=codeGenerator::ident6+var.varName+" = "+"e._value._"+var.varName+";\n";
              }*/

              line+=codeGenerator::ident6+v.varName+" = "+"event._value._"+v.varName+";\n";
                ...
                for(auto varPH : map){
                  ...
                  //Check if a variable is in the expression replaced by the placeholder
                  if (std::find(varPH.second.begin(), varPH.second.end(), v.varName) != varPH.second.end()){
                    auto placeholder = varPH.first;
                    usedPlaceholders.push_back(placeholder);
                    line+=codeGenerator::ident6+"const bool "+placeholder+" = ";

                    std::string expression = oden::prop2String(*(parsedFormula.second[placeholder]));

                    //If the expression is of type float == var use the floatEquals method
                    /*if((v.type).compare("float") == 0 && expression.find("==") != std::string::npos){
            ...
            line="";
            int i = 0;
            bindings.insert(std::pair<std::string,std::vector<checkerVar>>(checkerName, std::vector<checkerVar>()));
          
            for (auto v: varList){
    
              bindings[checkerName].push_back(v);

              line+="void "+checkerName+"::addEvent_var"+std::to_string(i)+"(ros::Time ts, ";
              line+=v.type+" value){\n";
              line+=codeGenerator::ident1+"if(checkerPhase == pausing && ts > timestampToReach){\n";
              line+=codeGenerator::ident2+"sendBufferClient(_remoteHandlerName);\n";
              line+=codeGenerator::ident2+"checkerPhase = paused;\n";
              line+=codeGenerator::ident1+"}\n";
              line+=codeGenerator::ident1+"else{\n";
              line+=codeGenerator::ident2+"addEvent_mutex.lock();\n";
              line+=codeGenerator::ident2+"vbuff.push_back(Event(ts, Value(value, "+std::to_string(i)+"),";
  ...
  bool generateHeader(codeGenerator::SpotAutomata& fsm,std::vector<checkerVar>& varList,std::string& checkerName){
    ...
    if(dst.fail()){
      std::cout<<"Error: could not open output/checkers/include/"<<checkerName<<".hh"<<std::endl;
      return false;
    }
    ...
    while (getline(src, line)){
        ...
        if(line.compare("$init") == 0){
          ...
          for(auto prop : fsm->ap()){
            line+=codeGenerator::ident1+"bool last_"+prop.ap_name()+ " = false;\n";
          }
        }
        ...
        //Code for defining the Value union based on checker's variables
            ...
            for(int i = 0; i< varList.size(); i++){
                ...
                if(usedTypes.find(var.type) == usedTypes.end()){
                   line+=codeGenerator::ident2+"Value(const "+var.type+" var, size_t id)";
                }
               
                //There are at least 2 variables of the same type and the current variable has not already been used
                if(count[var.type] > 1 && usedTypes.find(var.type) == usedTypes.end()){
                  line+="{\n";
                  line+=codeGenerator::ident3+"switch (id) {\n";
                  for(int j = i; j<varList.size(); j++){
                    auto var2 = varList[j];
                    if((var2.type).compare(var.type) == 0){
                      line+=codeGenerator::ident4+"case "+std::to_string(j)+":\n";
        ...
        //initial state
            ...
            for(auto var: varList){
             ...
             line+="#define INIT_"+checkerName+" "+std::to_string(fsm->get_init_state_number())+"\n";
            ...
            int i = 0;
            for (auto v: varList){
              line+=codeGenerator::ident1+"void addEvent_var"+std::to_string(i)+"(ros::Time ts, ";
              line+=v.type+" value);\n";
              i++;
            }
  ...
  bool generateHandler(rapidxml::XmlNodeList& checkers, int nVars[], std::string handlerName, std::string migrateTo){
    ...
    if(dst.fail()){
      std::cout<<"Error: could not open output/ver_env/src/ver_env.cpp"<<std::endl;
      return false;
    }
    ...
    while (getline(src, line)){
      ...
      if(line.compare("$addCheckers") == 0){
        line="";
        line+=codeGenerator::ident1+"std::string handlerName = "+"\""+handlerName+"\""+";\n";
        ...
        for(auto ch: checkers){
          ...
          line+=codeGenerator::ident1;
          line+="chs[\""+checkerName+"\"] = new "+checkerName+"("+std::to_string(nVars[i])+",rand() % 2 + 1,";
      ...
      //Generate callback functions
        ...
        for(auto e: checkersMsg){
          ...
          line+=codeGenerator::ident2+"if(checker -> getPhase() != Checker::Phase::paused){\n";

          for (auto ch: checkerList){
            int i = 0;
            line+=codeGenerator::ident3+"if(dynamic_cast<"+ch+"*>(checker) != NULL){\n";
            line+=codeGenerator::ident4+ch+" *ch = dynamic_cast<"+ch+"*>(checker);\n";
            
            
            auto vars = bindings[ch];
            std::string msgField = "";
            for(auto v:vars){
              if(msgType.compare(v.msgType) == 0 && topicName.compare(v.topicName) == 0){
                msgField = v.msgField;
                line+=codeGenerator::ident4+"ch->addEvent_var"+std::to_string(i)+"(msg->header.stamp, msg->"+msgField+");\n";
              }
              i++;
            }
            line+=codeGenerator::ident3+"}\n";
      ...
      //Declare a spinner thread and a queue for each callback 
        ...
        for(int i = 0; i< callbackNum;i++){
          ...
          line+=codeGenerator::ident1+"queue_"+n+" = new ros::CallbackQueue;\n";
          line+=codeGenerator::ident1+"n.setCallbackQueue(queue_"+n+");\n";
          line+=codeGenerator::ident1+"sub_"+n+" = new ros::Subscriber;\n";
          line+=codeGenerator::ident1+"*sub_"+n+" = n.subscribe("+"\""+callbackTopic[callbackName]+"\", ";
          line+=std::string("1000, ")+callbackName+", ros::TransportHints().tcpNoDelay());\n";
          line+=codeGenerator::ident1+"thread_"+n+" = new std::thread([&queue_"+n+", &spinner_"+n+"]() {\n";
          line+=codeGenerator::ident2+"spinner_"+n+" = new ros::SingleThreadedSpinner;\n";
        ...
        line="";
        for(int i = 0; i< callbackNum;i++){
          line+=codeGenerator::ident1+"thread_"+std::to_string(i)+"->join();\n";
        }
        line+=codeGenerator::ident1+"keepPolling = false;\n\n";

        for(int i = 0; i< callbackNum;i++){
          line+=codeGenerator::ident1+"delete thread_"+std::to_string(i)+";\n";
          line+=codeGenerator::ident1+"delete sub_"+std::to_string(i)+";\n";
  ...
  bool generateHeaderHandler(rapidxml::XmlNodeList& checkers){
    ...
    if(dst.fail()){
      std::cout<<"Error: could not open output/checkers/include/include_checkers.hh"<<std::endl;
      return false;
    }
  ...
  int main(int argc, char *args[]) {
  	...
  	if(argc < 2){
  		std::cout<<"Missing XML specification file, call as ./carmine <specification.xml>"<<std::endl;
  		return 0;
  	}
    ...
    //Each item in the array is the number o placeholders generated for each checker formula
    int nVars[checkers.size()];
    int i = 0;
    ...
    for (auto ch : checkers){
      ...
      if( generateCpp(fsm,varList,parsedFormula,checkerName) && generateHeader(fsm,varList,checkerName)){
		 std::cout<<"Successfully generated files for checker "<<checkerName<<std::endl;
	  }
	  else{
		 std::cout<<"Could not generate files for checker "<<checkerName<<std::endl;
		 return 1;
	  }
    ...
    if(! generateHeaderHandler(checkers)){std::cout<<"Could not generate header file for handler node"<<std::endl; return 1;}
    if(! generateHandler(checkers,nVars,handlerName,migrateTo)){std::cout<<"Could not generate handler node"<<std::endl; return 1;}
