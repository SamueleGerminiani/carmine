...
void generateChecker(const SpotAutomata &aut, const std::string checkerName,
  ...
  for (unsigned state = 0; state < num_states; state++) {
    ...
    if (aut->state_is_accepting(state)) { // Current state is not an error state
      ...
      outstream << "\n"
                << codeGenerator::ident2 << "case " << state << ":"
                << std::endl;
      for (auto &edge : aut->out(state)) {
        ...
        if (!(stringF.compare("1") == 0 && state == edge.dst)) {
          ...
          outstream << codeGenerator::ident3 << "if(" << stringF << "){"
                    << std::endl;

          // Don't go in error state but stay in current state and return false
          if (aut->state_is_accepting(edge.dst) ==
              false) { // Next state is error state
            outstream << codeGenerator::ident4 << "return false;" << std::endl;
          }

          else {
            outstream << codeGenerator::ident4 << "NEXT_STATE = " << edge.dst
                      << ";" << std::endl;
          }
