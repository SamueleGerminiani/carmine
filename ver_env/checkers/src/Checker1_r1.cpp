...
void Checker1_r1::notifyFailure() {
    std::cout << ros::this_node::getName() << ": Checker " << _checkerName
              << " failed, resetting" << std::endl;
}

Checker1_r1::Checker1_r1(size_t nVars, size_t priority, std::string handlerName,
   ...
   _timerInstances[0];
   _timerCache[0];
   _timeouts.push_back(20000);


    resetChecker();
}

Checker1_r1::~Checker1_r1() {
    delete[] _pbuff;
    delete[] _order;
}

void Checker1_r1::clearData() {
    _vbuff.clear();
    _evalIndex_p = 0;
    _index_p = 0;
    _eventsInBuffer = 0;
...
bool Checker1_r1::eval() {
   ...
   while (1) {
      if (std::chrono::duration_cast<std::chrono::microseconds>( std::chrono::steady_clock::now() - ohstart).count() >= 3000) {
   ...
   if (!eval_Checker1_r1(_last_p0, _last_p1, 0)){
      notifyFailure();
      eval_Checker1_r1(0, 0, 1);
   }
...
inline bool Checker1_r1::eval_Checker1_r1(bool p0, bool p1, bool const reset){
   if(reset){
      resetChecker();
      return 1;
   }
...
void Checker1_r1::reorder(bool forceReorder) {
   ...
   static double ef13;
        ...
        for (auto event : _vbuff) {
            switch (event._type) {
               case 0:{
                  ef12 = event._value._ef12;
                  const bool p0 = ((ef12 != 0) && (ef13 != 0));
                  const bool p1 = ((ef12 == 0) && (ef13 == 0));
                  assign<bool>(_pbuff, _index_p, p0, p1);
                  assign<bool>(_order, _index_p, 1, 1);
               }
               break;
               case 1:{
                  ef13 = event._value._ef13;
                  const bool p0 = ((ef12 != 0) && (ef13 != 0));
                  const bool p1 = ((ef12 == 0) && (ef13 == 0));
                  assign<bool>(_pbuff, _index_p, p0, p1);
                  assign<bool>(_order, _index_p, 1, 1);
...
void Checker1_r1::addEvent_ef12(ros::Time ts, double value){
      _addEvent_mutex.lock();
      _last_msg_ts = ts;
      _numberOfAddEvent++;
...
void Checker1_r1::addEvent_ef13(ros::Time ts, double value){
      _addEvent_mutex.lock();
      _last_msg_ts = ts;
      _numberOfAddEvent++;
...
void Checker1_r1::addTimerValue(size_t timerID) {
    _timerInstances[timerID].push_back(ros::Time::now().toSec() * 1000);
}
void Checker1_r1::popTimerInst(size_t timerID, size_t nToErase) {
...
bool Checker1_r1::getTimerValue(size_t timerID, size_t timerInstance,
                              bool isAss) {
    if (isAss) {
...
ros::Time Checker1_r1::migrateFromHandleTSbefore() {
    while (_last_msg_ts == ros::Time(0)) {
        ros::Duration(0.01).sleep();
    }
    return _last_msg_ts;
}
void Checker1_r1::migrateFromHandleTSAfter(const ros::Time &ts) {
    while (ts > _last_msg_ts) {
        ros::Duration(0.01).sleep();
    }
}
void Checker1_r1::migrateFromHandleData(ver_env::checkerData &res) {
    // buff
...
ros::Time Checker1_r1::migrateToHandleTS(const ros::Time &ts) {
...
void Checker1_r1::migrateToHandleData(ver_env::checkerData &res) {
    // buffers
