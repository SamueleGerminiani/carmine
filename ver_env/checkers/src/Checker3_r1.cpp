...
void Checker3_r1::notifyFailure() {
    std::cout << ros::this_node::getName() << ": Checker " << _checkerName
              << " failed, resetting" << std::endl;
}

Checker3_r1::Checker3_r1(size_t nVars, size_t priority, std::string handlerName,
...
Checker3_r1::~Checker3_r1() {
    delete[] _pbuff;
    delete[] _order;
}

void Checker3_r1::clearData() {
    _vbuff.clear();
    _evalIndex_p = 0;
    _index_p = 0;
    _eventsInBuffer = 0;
...
bool Checker3_r1::eval() {
   ...
   if (!eval_Checker3_r1(_last_p0, _last_p1, 0)){
      notifyFailure();
      eval_Checker3_r1(0, 0, 1);
   }
...
inline bool Checker3_r1::eval_Checker3_r1(bool p0, bool p1, bool const reset){
   if(reset){
      resetChecker();
      return 1;
   }
...
void Checker3_r1::reorder(bool forceReorder) {
...
void Checker3_r1::addEvent_pos0(ros::Time ts, double value){
      _addEvent_mutex.lock();
      _last_msg_ts = ts;
      _numberOfAddEvent++;
...
void Checker3_r1::addTimerValue(size_t timerID) {
    _timerInstances[timerID].push_back(ros::Time::now().toSec() * 1000);
}
void Checker3_r1::popTimerInst(size_t timerID, size_t nToErase) {
...
bool Checker3_r1::getTimerValue(size_t timerID, size_t timerInstance,
                              bool isAss) {
    if (isAss) {
...
ros::Time Checker3_r1::migrateFromHandleTSbefore() {
    while (_last_msg_ts == ros::Time(0)) {
        ros::Duration(0.01).sleep();
    }
    return _last_msg_ts;
}
void Checker3_r1::migrateFromHandleTSAfter(const ros::Time &ts) {
    while (ts > _last_msg_ts) {
        ros::Duration(0.01).sleep();
    }
}
void Checker3_r1::migrateFromHandleData(ver_env::checkerData &res) {
    // buff
...
ros::Time Checker3_r1::migrateToHandleTS(const ros::Time &ts) {
...
void Checker3_r1::migrateToHandleData(ver_env::checkerData &res) {
    // buffers
