...
void Checker2::notifyFailure() {
    std::cout << ros::this_node::getName() << ": Checker " << _checkerName
              << " failed, resetting" << std::endl;
}

Checker2::Checker2(size_t nVars, size_t priority, std::string handlerName,
...
bool Checker2::eval() {
   ...
   if (!eval_Checker2(_last_p0, _last_p1, 0)){
      notifyFailure();
      eval_Checker2(0, 0, 1);
   }
...
inline bool Checker2::eval_Checker2(bool p0, bool p1, bool const reset){
   if(reset){
      resetChecker();
      return 1;
   }
...
void Checker2::reorder(bool forceReorder) {
...
void Checker2::addEvent_vel0(ros::Time ts, double value){
      _addEvent_mutex.lock();
      _last_msg_ts = ts;
      _numberOfAddEvent++;
...
void Checker2::addTimerValue(size_t timerID) {
    _timerInstances[timerID].push_back(ros::Time::now().toSec() * 1000);
}
void Checker2::popTimerInst(size_t timerID, size_t nToErase) {
...
bool Checker2::getTimerValue(size_t timerID, size_t timerInstance,
                              bool isAss) {
    if (isAss) {
...
ros::Time Checker2::migrateFromHandleTSbefore() {
    while (_last_msg_ts == ros::Time(0)) {
        ros::Duration(0.01).sleep();
    }
    return _last_msg_ts;
}
void Checker2::migrateFromHandleTSAfter(const ros::Time &ts) {
    while (ts > _last_msg_ts) {
        ros::Duration(0.01).sleep();
    }
}
void Checker2::migrateFromHandleData(ver_env::checkerData &res) {
    // buff
...
ros::Time Checker2::migrateToHandleTS(const ros::Time &ts) {
...
void Checker2::migrateToHandleData(ver_env::checkerData &res) {
    // buffers
